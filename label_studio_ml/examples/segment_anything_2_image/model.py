...
def get_local_path(url, task_id):
    _image_name = url.split("/")[-1]
    _proj_id = url.split("/")[-2]
    ...
    #base_path = r"C:\Users\xray\Desktop\Workspace\Shuja\xis.edge\label_studio\core\metaDeta\media\upload"
    base_path = os.path.join(os.path.dirname(__file__),os.pardir,os.pardir,os.pardir,"core","metaDeta","media","upload")
    
    image_path = os.path.join(base_path,_proj_id, _image_name)
    print(image_path)
    return image_path


DEVICE = os.getenv("DEVICE", "cuda")
SEGMENT_ANYTHING_2_REPO_PATH = os.getenv("SEGMENT_ANYTHING_2_REPO_PATH", "sam22")
MODEL_CONFIG = os.getenv('MODEL_CONFIG', 'sam2_hiera_s.yaml')
MODEL_CHECKPOINT = os.getenv('MODEL_CHECKPOINT','sam2_hiera_small.pt')
...
class NewModel(LabelStudioMLBase):
    ...
    def predict(
        ...
        print(
            f"Point coords are {point_coords}, point labels are {point_labels}, input box is {input_box}"
        )
